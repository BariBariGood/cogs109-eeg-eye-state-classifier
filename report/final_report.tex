...
\author{%
  Ivan Del Rio \qquad Anish Kondamadugula \\[4pt]
...
\begin{abstract}
\noindent
We pick a single data-analysis approach from the COGS~109 course,
\textbf{classification}, and within it a single model,
\textbf{$k$-nearest neighbours (\knn{})}. We perform model
...
training data. A sanity-check analysis runs the same idea on three
...
instance-based KStar classifier under Weka's default 10-fold
cross-validation, which randomises and class-stratifies the folds ($3.2\%$
with default parameters; $2.7\%$ after tuning KStar's global-blend
parameter), while non-instance-based methods score far worse (Roesler
...
predictive: given an autocorrelated single-subject EEG recording and a
\knn{} classifier, how does the choice of cross-validation scheme change the
outcome of an otherwise identical model-selection procedure?
...
after outlier removal), so adjacent samples are nearly identical; the
label, being piecewise-constant over long runs, is even more persistent:
its autocorrelation is $\approx 0.997$ at lag~1, $\approx 0.85$ at lag~50
...
The consequence is direct: \emph{any} CV protocol that splits samples
uniformly at random leaves each test sample with a near-duplicate in its own
training set, so a model that predicts from per-sample similarity (\knn{} at
$k=1$) will appear highly accurate while merely memorising, not learning a
generalisable voltage-to-label rule.
...
\noindent The hypothesis has two parts: (i) the honest accuracy beats the
majority baseline, and (ii) the shuffled estimate is inflated relative to a
leakage-resistant estimate. We test both.
...
The COGS~109 course covers several broad modelling approaches (linear,
...
2021), making it ideal for a clean single-axis hyperparameter sweep. Second,
and most importantly for this study, because \knn{} predicts from per-sample
...
\begin{enumerate}[leftmargin=1.5em,itemsep=2pt,topsep=2pt]
  \item \textbf{Outlier removal.} We drop the four samples with at least
    one channel whose global standardised value exceeds $4$ in magnitude
...
near class-pure (e.g.\ one chunk at $\sim$20\% class-0 vs.\ the training
partition's $\sim$46.5\% class-0). Per-fold accuracy variance therefore explodes and the mean can
...
successive chunk along the sorted class-1-proportion distribution, so every fold's class
balance is within $\sim$1.5\pp{} of the training partition's $\sim$46.5\%
class-0 ($\sim$53.5\% class-1) balance.
...
hyperparameter, giving one accuracy per scheme. \textbf{PCA$\to$LDA} sweeps
...
Figure~\ref{fig:ksweep} plots \knn{} mean
...
$k=1$ to $\sim$0.84 at $k=201$, near-perfect everywhere. The naive
...
The key result is that \textbf{all three schemes select the same
...
(naive blocked) to $97.3\%$ (shuffled), a $47\pp$ spread, despite an
identical procedure, model, data, and selected $k$.
...
local. Under \emph{shuffled} CV the curve is anomalously flat and high
...
$>$99.8\% probability (only $23$ label changes across $14{,}976$ samples); under
...
\begin{table}[t]
  ...
  $\sim$3.5--8$\times$ larger than any other classifier in the palette.}
  \label{tab:gaps}
...
sample, and its gap is $\sim$3.5--8$\times$ larger than any other.
...
occipital channels rank well below these leaders (O1 negligible, O2 only
modest). We therefore do
...
The primary estimate is the honest \emph{within-recording} cross-validated
...
in the study (genuinely future data, separated from training by the
...
attributable purely to the CV scheme rather than the model
(\S\ref{sec:leakgap}). Part~(i)
...
voltage-to-state rule. On a single-subject,
...
their best results with the instance-based learner KStar ($\approx 97\%$)
under Weka's default (randomised, class-stratified) 10-fold CV while
non-instance-based methods score far worse
...
\section*{References}
{\small
\begin{itemize}[leftmargin=1.5em,label={},itemsep=2pt,topsep=1pt]
